...
void Game::GameLoop()
{
    Mix_PlayMusic(Mix_LoadMUS("sound/theme.mp3"), -1);
...
void Game::Update()
{
    ...
    if (next == Block::food)
    {
        Mix_PlayChannel(-1, Mix_LoadWAV("sound/eat.wav"), 0);
        score++;
        ReplaceFood();
        GrowBody(1);
    }
    // check chết(tự cắn vào thân)
...
void Game::Render()
{
    ...
    else       SDL_SetRenderDrawColor(renderer, 255, 204, 0, 255);
    SDL_RenderFillRect(renderer, &block);
